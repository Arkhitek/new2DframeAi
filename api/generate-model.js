...
\`\`\`
**3層3スパン例の解説:**
- **節点数**: 3層×3スパン×2 = 18節点（実際は12節点、柱脚と柱頭のみ）
- **部材数**: 3層×3スパン（柱）+ (3-1)×3スパン（梁）= 9 + 6 = 15部材
- **節点配置**: 1層(0,0)(6,0)(12,0)、2層(0,4)(6,4)(12,4)、3層(0,8)(6,8)(12,8)、4層(0,12)(6,12)(12,12)
- **部材配置**: 9本の柱 + 6本の梁
- **重要**: 3層3スパンは必ず12節点と15部材が必要です
...
- **節点番号と部材参照の重要ルール（最重要）:**
  ...
  - **複数層ラーメン構造の生成ルール（重要）:**
    - **層数とスパン数を正確に理解してください**: 「3層3スパン」は3つの層と3つのスパンを持つ構造です
    ...
    - **節点数の計算**: 層数×スパン数（柱脚）+ 層数×スパン数（柱頭）= 層数×スパン数×2
    - **部材数の計算**: 層数×スパン数（柱）+ (層数-1)×スパン数（梁）= 層数×スパン数×2 - スパン数
- **柱脚の境界条件に関する重要なルール:**
...
- **構造生成時の注意事項（必須確認事項）:**
  ...
  - **複数層・複数スパン構造の生成時は特に注意:**
    ...
    - 節点数は層数×スパン数×2（柱脚と柱頭）になります
    - 部材数は層数×スパン数（柱）+ (層数-1)×スパン数（梁）になります
    - 例：「3層3スパン」なら12節点と15部材が必要です

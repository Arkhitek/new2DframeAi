...
**4層ラーメン構造の例（4層4スパン）:**
節点配置: 各層に5本の柱（4スパン+1）、4層 = 20節点
部材配置: 15本の柱 + 16本の梁 = 31部材
...
**重要**: 上記の例は3層までしか梁がありません。4層構造では必ず4層目の梁も配置してください。
...
**4層構造の特別注意事項:**
- 4層構造では5層分の節点（25節点）が必要です（4層目の梁を配置するため）
- 4層目の柱は3層目から4層目への垂直部材です（5本）
- 4層目の梁は4層目から5層目の節点間を水平に接続する部材です（4本）
- 全ての層で柱と梁を完全に配置してください（欠落させない）
- 階高3.5mの場合、Y座標は0, 3.5, 7, 10.5, 14mに配置（5層分）
- スパン6mの場合、X座標は0, 6, 12, 18, 24mに配置（5列）
- 4スパン = 5列の節点が必要です（スパン数+1）
- 4層構造の部材数: 20本の柱 + 16本の梁 = 36部材`;
        } else {
            // 中小規模構造の例
            prompt += `

...
**トラス構造の例（スパン15m、高さ3m）:**
- 7節点、10部材（上弦3本+下弦4本+斜材6本）
- 上弦材：3→5→7（高さ3m）
- 下弦材：1→4→6→2（高さ0m）
- 斜材：1→3,3→4,4→5,5→6,6→7,7→2
- 全接合：ピン接合（"pinned"）`;
...
- 節点番号は配列順序で決まる（1から開始）
- 部材のiとjは存在する節点番号のみ参照
- 断面性能値不明時は一般的な鋼材断面値を仮定
- 座標系：右方向X軸正、上方向Y軸正

**境界条件設定ルール:**
- 地面接点（Y=0）：固定支点（"x"）またはピン支点（"p"）
- 中間節点：自由（"f"）
- 支点種類："x"（固定）、"p"（ピン）、"r"（ローラー）、"f"（自由）

**大規模構造（4層4スパン以上）の特別ルール:**
- 全柱と梁を必ず配置
- 柱：各層で同じX座標に垂直配置
- 梁：各層で柱間を水平接続
- 部材数確認：節点数と部材数が計算式と一致
- 節点番号連続性：1から順番に番号付け`;
